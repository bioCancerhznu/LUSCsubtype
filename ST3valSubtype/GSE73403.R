...
traindata <- read.csv(file = "dataTain.csv", row.names = 1)
...
testdata <- testdata[which(rownames(testdata) %in% names(traindata)),]
...
pdf(file = "survGSE73403.pdf", height = 5, width = 5)
print(ggsurv)
dev.off()
